...
from pyrogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, InputMediaDocument, InputMediaPhoto

from config import env_vars
from img2pdf.core import fld2pdf, fld2thumb
from text2pdf.core import text2pdf
from plugins import *
import os
...
from converter.cbz2pdf import CBZ2PDF
from converter.epub2pdf import EPUB2PDF
...
plugin_dicts: Dict[str, Dict[str, MangaClient]] = {
	...
	" Novel ": {
		...
		"NovelHall": NovelHallClient(),
		"WuxiaWorld": WuxiaWorldClient(),
		"ReadNovel": ReadNovelClient(),
	}
}
...
async def auto_handle_on(client: Client, message: Message):
	user_id = str(message.from_user.id)
	
	if user_id not in cf:
		cf[user_id] = {}
	
	cf[user_id]["thumb"] = message.photo.file_id 
	await message.reply_text("**Doned**")
	
@bot.on_message(filters=filters.command(['us', 'setting']))
async def user_info_cmds(app, message):
	bu = [
	...
	thumb = cf.get(user_id, {}).get('thumb', None)
	caption = cf.get(user_id, {}).get('caption', None)
	...
	banner = "True" if f_banner else "None"
	thumbnali = "True" if thumb else "None"
	lastb = "True" if l_banner else "None"
	
	if not thumb:
		thumb = "https://graph.org/file/aa477c9c5ba182829628b.jpg"
	try:
...
async def converter_handeler(client, message):
	file_name = message.document.file_name
	if file_name.endswith(".cbz"):
		await CBZ2PDF(client, message)
	elif file_name.endswith(".epub"):
		sts = await message.reply_text(" EPUB Found ")
		await asyncio.sleep(10)
		await sts.delete()
		await EPUB2PDF(client, message)


@bot.on_callback_query(filters.regex("close"))
async def close_o_handler(app, query):
	await query.message.delete()
	await query.answer()


@bot.on_callback_query(filters.regex(r"^ui"))
async def User_info_handler(app, query):
	bu = [
		[InlineKeyboardButton("* Thumb *", callback_data="ui_thumb"), InlineKeyboardButton("* Caption *", callback_data="ui_caption")],
		[InlineKeyboardButton("1st Banner", callback_data="ui_1_banner"), InlineKeyboardButton("2nd Banner", callback_data="ui_l_banner")],
		[InlineKeyboardButton("File Name", callback_data="ui_1_file_name")],
	...
	if query.data == "ui_thumb":
		if thumb:
			await query.message.edit_media(InputMediaPhoto(thumb))
		
		thumb_button = [
			[InlineKeyboardButton("* Set/Change Thumb *", callback_data="ui_thumb_set")],
			[InlineKeyboardButton("* Delete Thumb *", callback_data="ui_thumb_del")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		]
		await query.message.edit_reply_markup(InlineKeyboardMarkup(thumb_button))
	
	elif query.data == "ui_thumb_set":
		thumb_button = [
			[InlineKeyboardButton("* Set/Change Thumb *", callback_data="ui_thumb_set")],
			[InlineKeyboardButton("* Delete Thumb *", callback_data="ui_thumb_del")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		...
		msg = await app.send_message(int(user_id), text="Send Your Thumbnali")
		...
		try:
			...
			cf[user_id]["thumb"] = data_id.photo.file_id
			sync()
			
			await query.message.edit_media(InputMediaPhoto(data_id.photo.file_id))
			await query.message.edit_caption(User_TXT.format(user_id, banner, lastb, caption, "True", file_name),
		...
		if thumb:
			cf[user_id]["thumb"] = None
			sync()
			
			try: await query.message.edit_media(InputMediaPhoto("https://graph.org/file/aa477c9c5ba182829628b.jpg"))
			except: pass
			await query.message.edit_caption(User_TXT.format(user_id, banner, lastb, caption, "None", file_name),
							 reply_markup=InlineKeyboardMarkup(thumb_button))
		else:
			return await query.answer("You Havenot Thumb")
	
	elif query.data == "ui_caption":
		caption_button = [
			[InlineKeyboardButton("* Set/Change Caption *", callback_data="ui_cap_set")],
			[InlineKeyboardButton("* Delete Caption *", callback_data="ui_cap_del")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		]
		await query.message.edit_reply_markup(InlineKeyboardMarkup(caption_button))
	
	elif query.data == "ui_cap_set":
		caption_button = [
			[InlineKeyboardButton("* Set/Change Caption *", callback_data="ui_cap_set")],
			[InlineKeyboardButton("* Delete Caption *", callback_data="ui_cap_del")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		...
		msg = await app.send_message(int(user_id), text="Send Your Caption!!. \n<pre>{}: File Name</pre>")
		try:
			data_id = await app.listen(
				chat_id=int(user_id),
				timeout=60,
			)
			cf[user_id]["caption"] = data_id.text
			sync()
			
			await query.message.edit_caption(User_TXT.format(user_id, banner, lastb, data_id.text, thumbnali, file_name), reply_markup=InlineKeyboardMarkup(caption_button))
			await msg.delete()
			await data_id.delete()
		except Exception as err:
			await msg.edit(err)
	
	elif query.data == "ui_cap_del":
		caption_button = [
			[InlineKeyboardButton("* Set/Change Caption *", callback_data="ui_cap_set")],
			[InlineKeyboardButton("* Delete Caption *", callback_data="ui_cap_del")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		...
		if caption:
			cf[user_id]["caption"] = None
			sync()
			await query.message.edit_caption(User_TXT.format(user_id, banner, lastb, "None", thumbnali, file_name), reply_markup=InlineKeyboardMarkup(caption_button))
		else:
			return await query.answer("You Havenot Caption")
	
	elif query.data == "ui_1_file_name":
		fn_button = [
			[InlineKeyboardButton("* Set/Change File Name *", callback_data="ui_set_name")],
			[InlineKeyboardButton("* Delete File Name *", callback_data="ui_del_name")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		]
		await query.message.edit_reply_markup(InlineKeyboardMarkup(fn_button))
	
	elif query.data == "ui_set_name":
		fn_button = [
			[InlineKeyboardButton("* Set/Change File Name *", callback_data="ui_set_name")],
			[InlineKeyboardButton("* Delete File Name *", callback_data="ui_del_name")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		...
		msg = await app.send_message(int(user_id), text="Send Your File Name\n\n{name}: Chapter Name\n{num}: Chapter Num")
		try:
			data_id = await app.listen(
				chat_id=int(user_id),
				timeout=60,
			)
			cf[user_id]["file_name"] = data_id.text
			sync()
			
			await query.message.edit_caption(User_TXT.format(user_id, banner, lastb, caption, thumbnali, data_id.text), reply_markup=InlineKeyboardMarkup(fn_button))
			await msg.delete()
			await data_id.delete() 
			
		except Exception as err:
			await msg.edit(err)
			try: await data_id.delete() 
			except: await data_id.delete() 
	
	elif query.data == "ui_del_name":
		fn_button = [
			[InlineKeyboardButton("* Set/Change File Name *", callback_data="ui_set_name")],
			[InlineKeyboardButton("* Delete File Name *", callback_data="ui_del_name")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		...
		if file_name:
			cf[user_id]["file_name"] = None
			sync()
			
			await query.message.edit_caption(User_TXT.format(user_id, banner, lastb, caption, thumbnali, "None"), reply_markup=InlineKeyboardMarkup(fn_button))
		else:
			return await query.answer("You Havenot File Name")
	
	elif query.data == "ui_1_banner":
		if f_banner:
			try: await query.message.edit_media(InputMediaPhoto(f_banner))
			except: pass
		
		thumb_button = [
			[InlineKeyboardButton("* Set/Change Banner *", callback_data="ui_1_banner_set")],
			[InlineKeyboardButton("* Delete Banner *", callback_data="ui_1_banner_del")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		]
		
		await query.message.edit_reply_markup(InlineKeyboardMarkup(thumb_button))
	
	elif query.data == "ui_1_banner_set":
		thumb_button = [
			[InlineKeyboardButton("* Set/Change Banner *", callback_data="ui_1_banner_set")],
			[InlineKeyboardButton("* Delete Banner *", callback_data="ui_1_banner_del")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		...
		try:
			...
			cf[user_id]["f_banner"] = data_id.photo.file_id
			sync()
			
			await query.message.edit_media(InputMediaPhoto(data_id.photo.file_id))
			await query.message.edit_caption(User_TXT.format(user_id, "True", lastb, caption, thumbnali, file_name),
							 reply_markup=InlineKeyboardMarkup(thumb_button))
	...
	elif query.data == "ui_1_banner_del":
		thumb_button = [
			[InlineKeyboardButton("* Set/Change Banner *", callback_data="ui_1_banner_set")],
			[InlineKeyboardButton("* Delete Banner *", callback_data="ui_1_banner_del")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		...
		if f_banner:
			cf[user_id]["f_banner"] = None
			sync()
			
			await query.message.edit_caption(User_TXT.format(user_id, "None", lastb, caption, thumbnali, file_name), reply_markup=InlineKeyboardMarkup(thumb_button))
		else:
			return await query.answer("You Havenot Banner")
	
	elif query.data == "ui_l_banner":
		if l_banner:
			try: await query.message.edit_media(InputMediaPhoto(l_banner))
			except: pass
	...
	elif query.data == "ui_l_banner_set":
		thumb_button = [
			[InlineKeyboardButton("* Set/Change Banner *", callback_data="ui_l_banner_set")],
			[InlineKeyboardButton("* Delete Banner *", callback_data="ui_l_banner_del")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		...
		try:
			...
			cf[user_id]["l_banner"] = data_id.photo.file_id
			sync()
			
			await query.message.edit_media(InputMediaPhoto(data_id.photo.file_id))
			await query.message.edit_caption(User_TXT.format(user_id, banner, "True", caption, thumbnali, file_name), reply_markup=InlineKeyboardMarkup(thumb_button))
			await msg.delete()
			await data_id.delete()
	...
	elif query.data == "ui_l_banner_del":
		thumb_button = [
			[InlineKeyboardButton("* Set/Change Banner *", callback_data="ui_l_banner_set")],
			[InlineKeyboardButton("* Delete Banner *", callback_data="ui_l_banner_del")],
			[InlineKeyboardButton("<= Back", callback_data="ui_back")],
		...
		if l_banner:
			cf[user_id]["l_banner"] = None
			sync()
			
			await query.message.edit_caption(User_TXT.format(user_id, banner, "None", caption, thumbnali, file_name), reply_markup=InlineKeyboardMarkup(thumb_button))
		else:
			return await query.answer("You Havenot Banner")
		...
		if thumb:
			...
			await query.message.edit_media(InputMediaPhoto("https://graph.org/file/aa477c9c5ba182829628b.jpg"))
		
		await query.message.edit_caption(User_TXT.format(user_id, banner, "None", caption, thumbnali, file_name), reply_markup=InlineKeyboardMarkup(bu))
...
async def chapter_click(client, data, chat_id):
 ...
 logger.debug(f'Put chapter {chapters[data].name} to queue for user {chat_id} - queue size: {pdf_queue.qsize()}') """
	if int(pdf_queue.qsize()) < 150:
		await pdf_queue.put(chapters[data], int(chat_id))
		logger.debug(f"Put chapter {chapters[data].name} to queue for user {chat_id} - queue size: {pdf_queue.qsize()}")
	else:
		try: await bot.send_message(chat_id, text="<b>Bot Hit It Limit\n\nUse: @Manga_Downloaderx_bot</b>")
		except: pass
...
async def send_manga_chapter(client: Client, chapter, chat_id):
	...
	if chapter.novel == "True":
		pictures_folder = await chapter.client.set_pictures(chapter)
	else:
	...
	if file_name:
		...
		ch_name = str(file_name)
		ch_name = ch_name.replace("{num}", str(chap_num))
		ch_name = ch_name.replace("{name}", str(chap_name))
	else:
		ch_name = clean(f'{chapter.name} - {clean(chapter.manga.name, 25)}', 45)
	
	if caption:
		success_caption = caption.replace("{}", ch_name)
	else:
		success_caption = ch_name
	...
	if f_banner:
		if chapter.novel == "True":
			pass
			#f_banner_path = await client.download_media(f_banner)
		else:
			f_path = pictures_folder / 'fristbanner.jpg'
			
			f_path.parent.mkdir(parents=True, exist_ok=True)
			
			f_banner_path = await client.download_media(f_banner, file_name=f_path)
	
	if l_banner:
		if chapter.novel == "True":
			pass
			#l_banner_path = await client.download_media(l_banner)
		else:
			l_path = pictures_folder / 'l_banner.jpg'
			
			l_path.parent.mkdir(parents=True, exist_ok=True)
			
			l_banner_path = await client.download_media(l_banner, file_name=l_path)
	
	if chapter.novel == "True":
		try:
			pdf = await asyncio.get_running_loop().run_in_executor(None, text2pdf, pictures_folder.pictures, ch_name, f_banner_path, l_banner_path)
			if f_banner_path: os.remove(f_banner_path)
			if l_banner_path: os.remove(l_banner_path)
		except Exception as e:
			logger.exception(f'Error creating text2pdf for {chapter.name} - {chapter.manga.name}\n{e}')
			return await client.send_message(chat_id, f'There was an error making the pdf(text2pdf) for this chapter. '
		...
		try:
			...
			logger.exception(f'Error creating img2pdf for {chapter.name} - {chapter.manga.name}\n{e}')
			return await client.send_message(chat_id, f'There was an error making the pdf(img2pdf) for this chapter. '
							 f'Forward this message to the bot group to report the '
							 f'error.\n\n{error_caption}')
